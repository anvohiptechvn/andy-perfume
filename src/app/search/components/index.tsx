...
const SearchPage = () => {
  ...
  const query = useMemo<string>(
    () => (searchParams.get("q")?.toLowerCase() || "").trim().toLowerCase(),
    [searchParams]
  );

  const filteredProducts = useMemo<Perfume[]>(
    () =>
      (products as Perfume[]).filter(
        (product) =>
    ...
    <section className="container mx-auto my-6 flex flex-col gap-6 border px-1.5 md:px-0">
      {filteredProducts.length > 0 ? (
        <div className="flex flex-col gap-6 px-3 py-6">
          <h3 className="text-center text-lg text-[20px] font-medium">
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 py-10">
          <h3 className="text-lg text-[20px] font-medium">
