  ...
  return (
    ...
    >
      ...
      <div className="relative flex w-full flex-col items-start gap-1">
        <h3
          className={cn(
            "line-clamp-2 text-left text-sm font-medium",
            layout === "column" ? "h-10" : "mb-1"
          )}
        >
          {perfume.name}
        </h3>
        <p
          className={cn(
            "text-primary-default transform text-sm font-semibold transition-all duration-300",
            {
        ...
        {layout === "row" && (
          <p className="mt-4 line-clamp-3 text-left text-xs font-normal text-[#ACACAC] md:text-sm">
            {perfume.description}
          </p>
        )}
      </div>
      ...
      <div
        className={cn(
          "absolute bottom-3 left-3 flex translate-x-4 translate-y-4 scale-75 gap-2 opacity-0 transition-all duration-300",
          {
