...
const PerfumeFilters: React.FC<PerfumeFiltersProps> = ({
  ...
  const filteredBrands = useMemo(() => {
    ...
    return brandJson.data.filter(
      (br) =>
        br.key.toLowerCase().includes(formatSearchValue.toLocaleLowerCase()) ||
        br.label.toLowerCase().includes(formatSearchValue.toLocaleLowerCase())
    );
  }, [localBrandSearch]);
  ...
  return (
    <aside className="md:space-y-6">
      <aside className="border">
        {/* Title */}
        <button
          className="border-b p-2.5 flex items-center justify-between w-full"
          onClick={() => setFilterOpen((prev) => !prev)}
        >
          <div>
            <h2 className="w-fit text-lg font-bold uppercase">Bộ lọc</h2>
          ...
          <ChevronDown
            size={20}
            className={cn(
              "block md:hidden text-primary-default transition-all duration-200",
              filterOpen ? "rotate-180" : "rotate-0"
            )}
          />
        </button>

        {/* Transition dropdown based on filterOpen state */}
        <div
          className={cn(
            "transition-all duration-400 overflow-hidden md:max-h-fit md:opacity-100",
            filterOpen ? "max-h-fit opacity-100" : "max-h-0 opacity-0"
          )}
        >
          {/* Brand Filter */}
              ...
              <div className="space-y-2">
                {filteredBrands.map((brand) => (
                  <div key={brand.key} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      checked={
                        Array.isArray(params?.brand)
                          ? params.brand.includes(brand.key)
                          : (params?.brand || "").split(",").includes(brand.key)
                      }
                      onCheckedChange={(checked) => {
                        const currentBrands = Array.isArray(params?.brand)
                          ? params.brand
                          : (params?.brand || "").split(",").filter(Boolean);
                        let newBrands: string[];
                        if (checked) {
                          newBrands = Array.from(
                            new Set([...currentBrands, brand.key])
                          );
                        } else {
                          newBrands = currentBrands.filter(
                            (b: string) => b !== brand.key
                          );
                        }
                        ...
                        setParams((prev) => ({
                          ...prev,
                          brand:
                            newBrands.length > 0 ? newBrands.join(",") : null,
                        }));
                    ...
                    <Label
                      htmlFor={brand.key}
                      className="text-xs font-normal text-[#6F6F6F] md:text-sm"
                    >
                      {brand.label}
                    </Label>
                  </div>
