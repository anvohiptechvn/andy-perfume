...
const CategorySidebar: React.FC<CategorySidebarProps> = ({
  ...
  return (
    <aside
      className={cn(
        "h-fit border md:col-span-1",
        displayedBlogs ? "pb-2.5" : "border-b-0"
      )}
    >
      <div>
        <h3 className="border-b p-2.5 text-xs font-semibold md:text-sm">
          DANH MỤC
        </h3>
        <ul>
                    ...
                    {cat.children!.map((child, childIndex) => {
                      ...
                      return (
                        <li key={childIndex} className="px-2.5 py-[3px]">
                          <a
                            href={child.link!}
                            className="hover:text-primary-default text-xs text-[#222222] md:text-sm"
                          >
                            {child.name}
                          </a>
                        </li>
